   ...
   Vector2Int CheckNeighbours()
{
    ...
    {
        Vector2Int neighbour = currentCell; // Re-set neighbour to currentCell at the beginning of each loop
    ...
    void CarvePath(int x, int y)
{
    ...
    while (!deadEnd)
    {
        ...
        if (nextCell == currentCell)
        {
            ...
            {
                currentCell = path[path.Count - 1]; // Backtrack to the previous cell
                path.RemoveAt(path.Count - 1); // Remove the dead end from the path
            }
        }
        else
        {
            ...
            path.Add(currentCell); // Add the new cell to the path
        }
    }
}
...
public class MazeCell
{
   public bool visited;
    ...
    public bool rightWall = true; // Adicionado
    public bool bottomWall = true; // Adicionado
